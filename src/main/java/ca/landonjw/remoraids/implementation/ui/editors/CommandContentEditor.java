...
/**
 * The {@link IEditorUI} used for editing an existing {@link CommandContent}.
 *
 * @author landonjw
 * @since 1.0.0
 */
public class CommandContentEditor implements IEditorUI<CommandContent> {

	/** {@inheritDoc} */
	@Override
	public void open(@Nonnull IBossUI source, @Nonnull EntityPlayerMP player, @Nonnull CommandContent commandContent) {
		new Editor(source, player, source.getBossEntity(), commandContent).open();
	}

	class Editor extends BaseBossUI {

		/** The command content being edited. */
		 ...
		 * @param content    the command content being edited
		 */
		public Editor(@Nullable IBossUI source, @Nonnull EntityPlayerMP player, @Nonnull IBossEntity bossEntity, @Nonnull CommandContent content) {
