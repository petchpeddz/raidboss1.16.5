...
/**
 * The {@link IEditorUI} used for editing an existing {@link CurrencyContent}.
 *
 * @author landonjw
 * @since 1.0.0
 */
public class CurrencyContentEditor implements IEditorUI<CurrencyContent> {

	/** {@inheritDoc} */
	@Override
	public void open(@Nonnull IBossUI source, @Nonnull EntityPlayerMP player, @Nonnull CurrencyContent content) {
		new Editor(source, player, source.getBossEntity(), content).open();
	}

	class Editor extends BaseBossUI {

		/** The currency content being edited. */
		private CurrencyContent content;
		 ...
		 */
		public Editor(@Nullable IBossUI source, @Nonnull EntityPlayerMP player, @Nonnull IBossEntity bossEntity, @Nonnull CurrencyContent content) {
